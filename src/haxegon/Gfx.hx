...
class Gfx {
    ...
    public static var screen_width_mid: Int;
    public static var screen_height_mid: Int;
    ...
    * to scale X and Y seperately. */
    ...
    public static function change_tileset(tilesetname: String) {
    ...
    private static function imagealignx(x: Float): Float {
        if (x <= -5000) {
            ...
            if (t1 == 0 || (Math.abs(t1) < Math.abs(t2) && Math.abs(t1) < Math.abs(t3))) {
                return t1 + Gfx.screen_width_mid - Std.int(images[imagenum].width / 2);
    ...
    private static function imagealigny(y: Float): Float {
        if (y <= -5000) {
            ...
            t3 = y - BOTTOM;
            if (t1 == 0 || (Math.abs(t1) < Math.abs(t2) && Math.abs(t1) < Math.abs(t3))) {
                return t1 + Gfx.screen_height_mid - Std.int(images[imagenum].height / 2);
            } else if (t2 == 0 || ((Math.abs(t2) < Math.abs(t1) && Math.abs(t2) < Math.abs(t3)))) {
                return t2;
            } else {
    ...
    public static function grabtilefromscreen(tilenumber: Int, x: Float, y: Float) {
        if (currenttileset == -1) {
            throw("ERROR: In grabtilefromscreen, there is no tileset currently set. Use Gfx.change_tileset(\"tileset name\") to set the current tileset.");
            return;
        }

        settrect(x, y, tile_width(), tile_height());
    ...
    public static function grabtilefromimage(tilenumber: Int, imagename: String, x: Float, y: Float) {
        ...
        if (currenttileset == -1) {
            throw("ERROR: In grabtilefromimage, there is no tileset currently set. Use Gfx.change_tileset(\"tileset name\") to set the current tileset.");
            return;
        }
    ...
    public static function draw_tile(x: Float, y: Float, t: Int) {
        if (!clearscreeneachframe) if (skiprender && drawingtoscreen) return;
        if (currenttileset == -1) {
            throw("ERROR: No tileset currently set. Use Gfx.change_tileset(\"tileset name\") to set the current tileset.");
            return;
        }
        if (t >= numberoftiles()) {
            if (t == numberoftiles()) {
    ...
    public static function stop_animation(animationname: String) {
        animationnum = animationindex.get(animationname);
        animations[animationnum].reset();
    }

    public static function define_animation(animationname: String, tileset: String, startframe: Int, endframe: Int, delayperframe: Int) {
        if (delayperframe < 1) {
    ...
    public static function draw_animation(x: Float, y: Float, animationname: String) {
        if (!clearscreeneachframe) if (skiprender && drawingtoscreen) return;
        oldtileset = currenttilesetname;
        if (!animationindex.exists(animationname)) {
            throw("ERROR: No animated named \"" + animationname + "\" is defined. Define one first using Gfx.define_animation!");
            return;
        }
        animationnum = animationindex.get(animationname);
        change_tileset(animations[animationnum].tileset);
        ...
        change_tileset(oldtileset);
    }

    private static function tilealignx(x: Float): Float {
        if (x == CENTER) return Gfx.screen_width_mid - Std.int(tiles[currenttileset].width / 2);
    ...
    private static function tilealigny(y: Float): Float {
        if (y == CENTER) return Gfx.screen_height_mid - Std.int(tiles[currenttileset].height / 2);
    ...
    *  Hue is between 0-359, Saturation and Lightness between 0.0 and 1.0. */
    ...
    private static function initgfx(width: Int, height: Int, scale: Int) {
        ...
        screen_width_mid = Std.int(screen_width / 2); screen_height_mid = Std.int(screen_height / 2);
